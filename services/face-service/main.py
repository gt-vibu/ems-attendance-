...
and liveness/action verification server-side, using InsightFace (buffalo_l —
detection + landmarks + recognition bundled together) over ONNX Runtime.
...
~/.insightface/models/ — this requires outbound internet access the first
time only; after that it's cached on disk.
"""
...
app = FastAPI(title="Smart Teams Face Service", version="2.0.0")
...
# ---------------------------------------------------------------------------
# InsightFace's buffalo_l bundle includes the landmark_3d_68 submodel, which
# follows the standard iBUG/300W 68-point layout (jaw 0-16, right eyebrow
# 17-21, left eyebrow 22-26, nose 27-35, right eye 36-41, left eye 42-47,
# mouth 48-67) and automatically populates `face.pose` = [pitch, yaw, roll]
# in degrees (see model_zoo/landmark.py — pose is derived by fitting the
# predicted 3D landmarks against a canonical mean-shape template). We use
# the (x, y) projection of these 68 points for all 2D geometric ratios.
