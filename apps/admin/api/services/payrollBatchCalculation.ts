  ...
  await db.transaction(async (tx: any) => {
    ...
    for (const lineItem of lineItems as any[]) {
      ...
      for (const line of breakdown) {
        ...
        if (line.loanId) {
          const rows = await tx.select().from(schema.payrollLoans).where(eq(schema.payrollLoans.id, Number(line.loanId))).limit(1);
          const loan = rows[0];
          if (loan?.status === 'active') {
            const newBalance = Math.max(0, Number(loan.remainingBalance || 0) - Math.abs(Number(line.amount || 0)));
            await tx.update(schema.payrollLoans).set({ remainingBalance: newBalance, status: newBalance <= 0.01 ? 'closed' : 'active' }).where(eq(schema.payrollLoans.id, loan.id));
          }
        }
        if (line.advanceId) {
          const rows = await tx.select().from(schema.payrollAdvances).where(eq(schema.payrollAdvances.id, Number(line.advanceId))).limit(1);
          const advance = rows[0];
          if (advance?.status === 'active') {
            const newBalance = Math.max(0, Number(advance.remainingBalance || 0) - Math.abs(Number(line.amount || 0)));
            await tx.update(schema.payrollAdvances).set({ remainingBalance: newBalance, status: newBalance <= 0.01 ? 'closed' : 'active' }).where(eq(schema.payrollAdvances.id, advance.id));
          }
        }
        if (line.bonusId) {
