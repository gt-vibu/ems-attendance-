...
export async function buildReportPdf(
  ...
  return new Promise((resolve, reject) => {
    ...
    if (meta.signatureLine) {
      doc.moveDown(1.5);
      doc.fontSize(9).fillColor('#334155').font('Helvetica').text(meta.signatureLine);
    } else {
      doc.moveDown(1);
      doc.fontSize(8).fillColor('#94a3b8').font('Helvetica').text('Digitally generated — no signature required');
    }
    ...
    for (let i = 0; i < pageCount; i++) {
      ...
      if (meta.showWatermark) {
        ...
        doc.fontSize(48).fillColor('#e2e8f0').opacity(0.35).font('Helvetica-Bold').text(
          theme.watermark, 0, doc.page.height / 2 - 24, { align: 'center', width: doc.page.width },
        );
        doc.opacity(1);
        doc.restore();
      }
