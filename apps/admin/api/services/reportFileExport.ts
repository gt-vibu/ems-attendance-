...
export async function buildReportPdf(
  ...
  return new Promise((resolve, reject) => {
    ...
    const maxFooterY = doc.page.height - PAGE_MARGIN - 30;
    if (doc.y > maxFooterY) {
      doc.y = maxFooterY;
    }

    if (meta.signatureLine) {
      doc.fontSize(9).fillColor('#334155').font('Helvetica').text(meta.signatureLine, PAGE_MARGIN, doc.y, { width: pageWidth, lineBreak: false });
    } else {
      doc.fontSize(8).fillColor('#94a3b8').font('Helvetica').text('Digitally generated — no signature required', PAGE_MARGIN, doc.y, { width: pageWidth, lineBreak: false });
    }
    ...
    try {
      ...
      if (Array.isArray(buf) && buf.length > 1) {
        // If the last page in the buffer has no operations or is untouched, pop it
        const lastPg = buf[buf.length - 1];
        if (lastPg && (lastPg.operations === undefined || (Array.isArray(lastPg.operations) && lastPg.operations.length <= 1))) {
          buf.pop();
        }
      }
    } catch {
    ...
    const finalPageCount = (doc as any)._pageBuffer ? (doc as any)._pageBuffer.length : doc.bufferedPageRange().count;
    for (let i = 0; i < finalPageCount; i++) {
      ...
      doc.fontSize(7).fillColor('#94a3b8').text(
        `Confidential — generated by ${meta.tenantName} via Smart Teams EMS · Page ${i + 1} of ${finalPageCount}`,
        PAGE_MARGIN,
