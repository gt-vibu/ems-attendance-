...
export async function buildReportPdf(
  ...
  return new Promise((resolve, reject) => {
    ...
    // Watermark + page numbers + confidentiality footer on every page (using safe PDFKit API)
    ...
    for (let i = range.start; i < range.start + range.count; i++) {
      ...
      doc.fontSize(7).fillColor('#94a3b8').text(
        `Confidential — generated by ${meta.tenantName} via Smart Teams EMS · Page ${i + 1} of ${range.count}`,
        PAGE_MARGIN,
