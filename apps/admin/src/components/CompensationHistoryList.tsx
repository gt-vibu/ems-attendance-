...
export default function CompensationHistoryList({ history, emptyLabel }: { history: CompensationHistoryEntry[]; emptyLabel: string }) {
  ...
  return (
    <div className="space-y-4">
      {history.map((entry) => (
        <div key={entry.id} className="nexus-card rounded-xl p-4">
          ...
          {entry.fieldChanges.length === 0 ? (
            ...
            <div className="space-y-2">
              {entry.fieldChanges.map((change, i) => (
                <div key={i} className="flex flex-wrap items-center justify-between gap-2 rounded-xl bg-[var(--color-nexus-surface-alt)] px-4 py-2.5 text-xs">
