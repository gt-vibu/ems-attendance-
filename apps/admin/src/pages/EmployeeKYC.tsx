...
// services/face-service/main.py) were calibrated against, so this redesign
// changes only how the recording feels, not what's actually captured per
// action.
    ...
    <div className="min-h-screen premium-mesh-bg flex items-center justify-center p-6 font-sans relative overflow-hidden">
      ...
      >
        ...
        {/* Phase timeline — a moving playhead across every phase in this
            recording, replacing the old per-step dot indicator that only
            ever showed one step "active" at a time behind a blocking button. */}
        ...
        <div className="space-y-3">
          ...
          {/* Only shown before the very first recording. Once a recording has
              run, the flow is automatic: success navigates away, failure
              shows the "Redo those actions" link in the error banner above
              (redoFailedSteps) — that's the sole redo trigger, so this button
              doesn't need to (and shouldn't) also handle the redo case. */}
          {/* Camera never started (denied, dismissed, or revoked) — the
              ...
              its own settings UI: every other denial (closed the prompt,
              denied once this tab, revoked later) re-prompts on a fresh
              call, so this doubles as the retry path. */}
          ...
          {/* A hard failure with no specific failed-action list (e.g. a
              network error) — the "Redo those actions" link above only
              covers the failedActions case, so this is the retry path for
              everything else. Re-runs the full recording since we don't know
