    ...
    <div className={`w-full ${embedded ? 'p-0' : 'p-0'} font-sans text-slate-800`}>
      ...
      {activeCategory !== 'saved' && activeCategory !== 'schedules' && activeCategory !== 'compliance' && (
        <div>
            ...
            return (
              <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm mb-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-xl bg-indigo-600 text-white font-black text-xl flex items-center justify-center shrink-0 overflow-hidden shadow-xs">
                    {companyLogo ? (
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <h2 className="text-base font-extrabold text-slate-900 leading-tight">{companyName}</h2>
                      <span className="px-2 py-0.5 rounded text-[9px] font-bold uppercase font-mono bg-slate-100 text-slate-600 border border-slate-200">
                        Verified Enterprise Report
                      </span>
                    </div>
                    <p className="text-xs text-slate-500 mt-0.5 italic">"{companyTagline}"</p>
                  </div>
                </div>
                <div className="text-left md:text-right shrink-0 border-t md:border-t-0 pt-2 md:pt-0 border-slate-100">
                  <p className="text-xs font-bold text-slate-800 uppercase tracking-wider">Report Generated for {user.name || 'Account'}</p>
                  <p className="text-[10px] text-slate-400 font-mono mt-0.5">Date: {new Date().toLocaleDateString()} • Ref: REF-2026-REPORT</p>
                </div>
              </div>
            );
          })()}
          ...
          {/* Official Enterprise Report Footer */}
          {(() => {
            const companyName = localStorage.getItem('company_name') || user.tenantName || 'Smart Teams EMS';
            return (
              <div className="mt-6 pt-4 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between text-[10px] text-slate-400 font-mono gap-2">
                <div>
                  <span className="font-bold text-slate-700">{companyName}</span> • Enterprise Governance Console
                </div>
                <div>
                  Generated by Smart Teams EMS • Confidential
                </div>
                <div>
                  {new Date().toLocaleString()} • Page 1 of 1
                </div>
              </div>
            );
          })()}
