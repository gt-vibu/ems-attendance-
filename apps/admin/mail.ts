  ...
  export interface EmailResult {
    ...
    // claiming success.
    delivered: boolean;
    provider: 'resend' | 'sendgrid' | 'smtp' | 'simulated';
  }
  ...
  export async function sendEmail(options: EmailOptions): Promise<EmailResult> {
    ...
    // Chosen specifically because, unlike Resend, SendGrid allows verifying a
    // SINGLE sender email address (no domain/DNS ownership required) — see
    // Settings > Sender Authentication > Single Sender Verification in
    // SendGrid's dashboard. This exists because outbound SMTP was CONFIRMED
    // fully blocked on this Render deployment (verified with a real, non-
    // Gmail SMTP test host — smtp.ethereal.email — which also hit a
    // connection timeout, and by testing an entirely different Render
    // region, which made no difference either), so an HTTPS-based provider
    // is the only path to real delivery without buying a domain.
    // Deliverability caveat (real, not hypothetical): sending "from" a
    // @gmail.com address through a third-party relay like SendGrid can fail
    // Gmail's own DMARC alignment check, which some receiving mail servers
    // treat as a spam signal. Not a guaranteed failure, but worth knowing if
    // mail lands in spam rather than the inbox.
    if (sendgridKey && sendgridFrom) {
      try {
        ...
        console.log(`[SENDGRID SUCCESS] Email sent to ${to}`);
        return { delivered: true, provider: 'sendgrid' };
      } catch (err: any) {
        // SendGrid's API returns structured error detail in
        // err.response.body — far more useful for diagnosing a rejected
        // send (e.g. "from address does not match a verified Sender
        // Identity") than the bare error message alone.
        console.error(`[SENDGRID ERROR]`, err?.response?.body ? JSON.stringify(err.response.body) : (err?.message || err));
        // Fall through to SMTP / simulation instead of silently dropping the email.
      }
    }

    // --- OPTION 3: SMTP (Gmail App Password / Ethereal) ---
    if (smtpHost && smtpUser && smtpPass &&
      ...
      try {
        ...
        // Two previous attempts at forcing IPv4 (the `family: 4` transport
        // option, then process-wide dns.setDefaultResultOrder) both failed —
        // confirmed by real logs still showing `connect ENETUNREACH` against
        // an IPv6 address either way. That means nodemailer/its SMTP
        // connection layer isn't resolving the hostname through Node's
        // standard dns.lookup() path at all (it very likely does its own DNS
        // resolution and hands net.connect() a raw IP directly), so neither
        // approach had anything to influence. Resolving the A record
        // ourselves and handing nodemailer that literal IPv4 address as
        // `host` sidesteps whatever nodemailer does internally entirely —
        // there is no hostname left for it to resolve. `tls.servername` keeps
        // certificate validation targeting the real hostname (required — a
        // cert issued for smtp.gmail.com won't validate against a bare IP).
        ...
        try {
          ...
          console.warn(`[SMTP] Could not resolve ${smtpHost} to an IPv4 address, falling back to hostname:`, dnsErr?.message || dnsErr);
        }
